...
import {
    IconPlane, IconTrash, IconSwap, IconArrowUp, IconArrowDown, IconLocation, IconMaximize, IconDisk, IconFolder
} from './Icons';
...
export const FlightPlanPanel: React.FC<FlightPlanPanelProps> = ({
    ...
    const origin = waypoints.find(w => w.role === 'ORIGIN') || (waypoints.length > 0 ? waypoints[0] : null);
    const destination = waypoints.find(w => w.role === 'DESTINATION') || (waypoints.length > 1 ? waypoints[waypoints.length - 1] : null);
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar bg-[#0b0e14] p-4 space-y-2">
                    {waypoints.length === 0 ? (
                        ...
                        waypoints.map((wp, i) => {
                            ...
                            const isOrigin = i === 0;
                            const isDest = i === waypoints.length - 1 && waypoints.length > 1;

                            return (
                                <div key={wp.id} className={`relative group border rounded-xl p-3 transition-colors ${i === 0 ? 'bg-teal-500/10 border-teal-500/30' :
                                    ...
                                    {waypoints.map((wp, i) => {
                                        ...
                                        return (
                                            <tr key={wp.id} className="hover:bg-slate-800/30 transition-colors">
                                                ...
                                                <td className="p-4">
                                                    <span className={`text-[9px] font-bold px-2 py-1 rounded text-black ${i === 0 ? 'bg-teal-400' : (i === waypoints.length - 1) ? 'bg-purple-400' : 'bg-yellow-400'
                                                        }`}>
                                                        {i === 0 ? 'ORIGEM' : (i === waypoints.length - 1 ? 'DESTINO' : wp.type)}
                                                    </span>
                                                </td>
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-2">
                            {savedPlans.length === 0 ? (
                                ...
                                savedPlans.map((plan, i) => (
                                    <div key={i} className="flex items-center justify-between p-3 bg-slate-800/30 border border-slate-800 hover:bg-slate-800 hover:border-blue-500/30 rounded-lg group transition-colors cursor-pointer" onClick={() => {
                                        onLoadPlan(plan);
