  ...
  return (
    <>
      <section className="w-[420px] bg-[#0b0e14] border-r border-slate-800 flex flex-col z-[1001] shadow-2xl h-full relative">
        ...
        {/* 3. WAYPOINT LIST (FUNCIONAL & STYLED) */}
      ...
      {/* 4. MODALS: SAVE, LIBRARY, EXPANDED LOG */}
      {isSaveModalOpen && (
      ...
      {isLibraryOpen && (
        <div className="fixed inset-0 z-[3000] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
            <div className="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-900/50">
              <div className="flex items-center gap-2"><FolderOpen size={18} className="text-blue-400" /><h3 className="text-white font-black uppercase text-xs tracking-widest">Library</h3></div>
              <button onClick={() => setIsLibraryOpen(false)} className="text-slate-500 hover:text-white"><X size={24}/></button>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
              {savedPlans.length === 0 ? (
                <p className="text-slate-600 text-center py-10 text-[10px] font-black uppercase tracking-tighter italic">No saved plans found</p>
              ) : (
                savedPlans.map((plan) => (
                  <button key={plan.id} onClick={() => { onLoadPlan(plan); setIsLibraryOpen(false); }} className="w-full bg-slate-800/50 hover:bg-slate-800 border border-slate-700 p-4 rounded-xl flex justify-between items-center transition-all group">
                    <div className="text-left"><div className="text-white font-black text-xs uppercase">{plan.name}</div><div className="text-[9px] text-slate-500 font-bold uppercase">{plan.aircraft} • {plan.waypointsCount} Waypoints</div></div>
                    <div className="opacity-0 group-hover:opacity-100 bg-blue-600 text-white text-[9px] font-black px-3 py-1.5 rounded uppercase transition-all tracking-tighter">Load Plan</div>
                  </button>
                ))
              )}
            </div>
      ...
      {isLibraryOpen && (
        <div className="fixed inset-0 z-[5000] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#0b0e14] border border-slate-800 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
              {(!savedPlans || savedPlans.length === 0) ? (
                <p className="text-slate-600 text-center py-10 text-[10px] font-black uppercase italic">No saved plans</p>
              ) : (
                savedPlans.map((plan) => (
                  <div key={plan.id} className="w-full bg-slate-800/30 border border-slate-800/50 p-4 rounded-xl flex justify-between items-center group">
                    <div className="text-left">
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="text-[10px] uppercase font-black text-slate-500 tracking-widest border-b border-slate-800">
                    <th className="p-4">Report Point</th><th className="p-4">Coordinates</th><th className="p-4 text-right">Mag Track</th><th className="p-4 text-right">Distance</th><th className="p-4 text-right text-teal-400">ETE (Est.)</th><th className="p-4 text-right">Accumulated</th>
                  </tr>
                </thead>
