...
export const FlightPlanPanel: React.FC<FlightPlanPanelProps> = ({
  waypoints = [], flightSegments = [], plannedSpeed = 0, onPlannedSpeedChange,
  ...
  // LOG PARA DEBUG - Verifique o console se a tela apagar
  console.log("Saved Plans Data:", savedPlans);
  ...
  return (
    <>
            ...
            <Droppable droppableId="side-route-list">
              {(provided) => (
                <div {...provided.droppableProps} ref={provided.innerRef} className="divide-y divide-slate-800/50">
                  {Array.isArray(waypoints) && waypoints.map((wp, i) => (
      ...
      {/* LIBRARY MODAL - PROTEGIDO CONTRA TELA PRETA */}
      {isLibraryOpen && (
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
              {!Array.isArray(savedPlans) || savedPlans.length === 0 ? (
                ...
                savedPlans.map((plan, idx) => {
                  if (!plan) return null;
                  return (
                    <div key={plan.id || idx} className="w-full bg-slate-800/30 border border-slate-800/50 p-4 rounded-xl flex justify-between items-center group">
                      <div className="text-left">
                        <div className="text-white font-black text-xs uppercase">{plan.name || 'Unnamed'}</div>
                        <div className="text-[9px] text-slate-500 font-bold uppercase">
                          {plan.aircraft || 'N/A'} • {plan.waypointsCount || 0} pts
                        </div>
                      </div>
                      <button 
                        onClick={() => { onLoadPlan?.(plan); setIsLibraryOpen(false); }} 
                        className="bg-blue-600 hover:bg-blue-500 text-white text-[9px] font-black px-3 py-1.5 rounded uppercase"
                      >
                        Load
                      </button>
                    </div>
