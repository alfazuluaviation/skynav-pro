...
import { X, Search, FileText, ExternalLink, Plane, Loader2, MapPin } from 'lucide-react';
...
export const ChartsModal: React.FC<ChartsModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[3000] flex items-center justify-center">
      ...
      {/* Modal */}
      <div className="relative w-full max-w-2xl max-h-[85vh] bg-slate-900/95 backdrop-blur-xl border border-slate-700/50 rounded-2xl shadow-2xl overflow-hidden mx-4">
        {/* Header */}
            ...
            <div key={tipo} className="mb-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="text-sm font-medium text-slate-300">
                  {chartList[0]?.tipo_descr || tipo}
                </h3>
                <span className="text-xs text-slate-500">({chartList.length})</span>
              </div>
              <div className="space-y-2">
                {chartList.map((chart) => (
                  <button
                    key={chart.id}
                    onClick={() => openChart(chart.link)}
